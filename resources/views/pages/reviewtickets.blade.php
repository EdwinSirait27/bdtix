    ...
    <div class="px-4 space-y-6 pb-8 max-w-3xl mx-auto">
        ...
        @if(in_array($ticket->status, ['Closed', 'Overdue']) && !$ticket->review)
            <form method="POST" action="{{ route('reviewticketsfromhuman', request()->route('hash')) }}" class="space-y-6">
                ...
                <div class="bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-700 rounded-2xl p-6 shadow-xl">
                    <div class="flex items-center space-x-3 mb-6">
                        <div class="w-10 h-10 rounded-xl bg-yellow-500/20 flex items-center justify-center">
                            <svg class="w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20">
                                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.966a1 1 0 00.95.69h4.173c.969 0 1.371 1.24.588 1.81l-3.377 2.455a1 1 0 00-.364 1.118l1.287 3.966c.3.921-.755 1.688-1.54 1.118l-3.377-2.455a1 1 0 00-1.175 0l-3.377 2.455c-.784.57-1.838-.197-1.539-1.118l1.287-3.966a1 1 0 00-.364-1.118L2.98 9.393c-.783-.57-.38-1.81.588-1.81h4.173a1 1 0 00.95-.69l1.286-3.966z" />
                            </svg>
                        </div>
                        <div>
                    ...
                    <div class="mb-6">
                        ...
                        <select id="rating" name="rating" required
                            ...
                            @for ($i = 5; $i >= 1; $i--)
                                <option value="{{ $i }}" {{ old('rating') == $i ? 'selected' : '' }}>
                                    {{ $i }} ★ - {{ $i === 5 ? 'Excellent' : ($i === 4 ? 'Good' : ($i === 3 ? 'Average' : ($i === 2 ? 'Poor' : 'Very Poor'))) }}
                                </option>
                            @endfor
                        </select>
                        @error('rating')
            ...
            <div class="grid grid-cols-2 gap-4 pt-4 border-t border-slate-700">
                <div>
                    ...
                    <p class="text-sm font-medium text-slate-300">{{ optional($ticket->executor->employee)->employee_name ?? 'N/A' }}</p>
                </div>
                <div>
                ...
                <div>
                    ...
                    <p class="text-sm font-medium text-slate-300">{{ $ticket->finished ? \Carbon\Carbon::parse($ticket->finished)->format('d M Y, H:i') : '-' }}</p>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-green-900/20 to-emerald-900/20 border border-green-700/30 rounded-2xl p-6">
                ...
                <div class="bg-slate-900/50 rounded-xl p-4 space-y-3">
                    ...
                    <div>
                        ...
                        @endrole
                        @role('admin|executor')
                        <p class="text-xs text-slate-500 mb-2">Reviewed By : {{ $ticket->user->employee->employee_name }}</p>
                        @endrole
